...
export const COC_COLOR = 0xF4A723; // gold
export const WAR_COLOR = 0xE74C3C;
export const CWL_COLOR = 0x9B59B6;
export const RAID_COLOR = 0x2ECC71;
export const GAMES_COLOR = 0x3498DB;
...
export function clanEmbed(clan) {
  return baseEmbed(`${clan.name} [${clan.tag}]`)
    ...
    .addFields(
      ...
      { name: 'War Win Streak', value: `${clan.warWinStreak ?? 0}`, inline: true },
      { name: 'Trophy Points', value: `${clan.clanPoints ?? 0}`, inline: true },
...
export function playerEmbed(p) {
  const heroes = p.heroes?.map(h => `${h.name} Lv${h.level}`).join('\n') || 'None';
  return baseEmbed(`${p.name} [${p.tag}]`)
    .addFields(
      ...
      { name: '🛡️ Builder Trophies', value: `${p.builderBaseTrophies ?? 0}`, inline: true },
...
export function warEmbed(war, color = WAR_COLOR) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: 'State', value: stateLabel(state), inline: true },
