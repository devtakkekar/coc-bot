...
const CATEGORIES = {
  ...
  war: {
    ...
    commands: [
      ...
      { name: '/warattacks [tag]',      desc: 'Who hasn\'t used all their attacks yet' },
  ...
  admin: {
    ...
    description: '🔒 Requires **Administrator** permission. Hidden from non-admins.',
    commands: [
      { name: '/setchannel <event> <channel>', desc: 'Set notification channel for war, CWL, raids, games, announcements, admin alerts' },
      ...
      { name: '/cachestatus',                  desc: 'View age of all cached data' },
      { name: '/stats',                        desc: 'Bot uptime, memory, commands run, API calls, errors' },
    ],
  },
};
  ...
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions(
        ...
        { label: '🔐 Admin Commands', description: 'Channels, roles, stats, announcements',   value: 'admin',     emoji: '🔐' },
      )
  );
}
...
export default [
  {
    ...
    async execute(interaction) {
      ...
      collector.on('collect', async i => {
        ...
        const embed = selected === 'overview' ? buildOverviewEmbed() : buildCategoryEmbed(selected);
